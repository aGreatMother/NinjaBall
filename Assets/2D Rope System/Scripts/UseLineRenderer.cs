...
public class UseLineRenderer : MonoBehaviour {
	...
	void Update(){
		...
		{
			if(chains.Count>=tr.childCount)
				lineRend.SetVertexCount (chains.Count);
			else
				lineRend.SetVertexCount (tr.childCount + 2);
	...
	public void RemoveSingleChain(string name){
		...
		if (chains != null && chains.Count > 0) {
			...
			for(int i = 0; i < chains.Count; i++){
				...
				if ( name == chains[i].gameObject.name) {
					


					chains[i].parent = (brokenChainsHolder != null) ? brokenChainsHolder.transform : null;
					chains[i].GetComponent<Collider2D> ().isTrigger = true;
